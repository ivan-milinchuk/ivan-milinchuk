...
Dockerized Postgres pipeline (raw → staging → mart) producing `mart.mart_mrr_over_time` with monthly MRR movements (new, expansion, contraction, churn) + gross revenue churn rate.  
Repo: https://github.com/ivan-milinchuk/saas_churn_engine

### 🏠 STR Revenue Engine (InsideAirbnb)
Airflow-orchestrated STR analytics pipeline for multi-city CSV snapshots (Toronto/Vancouver/Montreal). Computes 30-day occupancy/ADR/revenue marts + flags undervalued listings.  
Repo: https://github.com/ivan-milinchuk/str-airbnb-revenue-engine
